...
function initialState(){ return { tasks:seedTasks, proposals:seedProposals, activity:[], draft:{...blankDraft}, editingTaskId:null, teamPoints:{'NOVA Lab':780,'Data Nomads':640,'Kórme':520,'Pixel Crew':410} }; }
function loadState(){
  try {
    ...
    if(parsed?.tasks&&parsed?.proposals&&parsed?.draft){
      ...
      return {...parsed,tasks,proposals,activity:Array.isArray(parsed.activity)?parsed.activity:[],draft:{...blankDraft,...parsed.draft},editingTaskId,teamPoints:parsed.teamPoints||initialState().teamPoints};
    }
...
function renderNav(){ const unread=state.activity.filter(item=>(item.audience||[]).includes(role)&&!(item.readBy||[]).includes(role)).length; const business=[['builder','Конструктор'],['tasks','Мои задачи'],['responses',`Отклики · ${state.proposals.filter(p=>p.status==='pending').length}`],['activity',`События${unread?` · ${unread}`:''}`]]; const student=[['catalog','Каталог'],['my-responses','Мои отклики'],['activity',`События${unread?` · ${unread}`:''}`]]; nav.innerHTML=(role==='business'?business:student).map(([id,label])=>`<button class="nav-button ${currentView===id?'active':''}" data-view="${id}">${label}</button>`).join(''); document.querySelectorAll('.role-button').forEach(button=>button.classList.toggle('active',button.dataset.role===role)); }
...
function render(){ renderNav(); if(currentView==='builder')renderBuilder(); if(currentView==='tasks')renderBusinessTasks(); if(currentView==='responses')renderResponses(); if(currentView==='catalog')renderCatalog(); if(currentView==='my-responses')renderMyResponses(); if(currentView==='activity')renderActivity(); }
...
function setTaskStatus(id,status){
  const task=state.tasks.find(item=>item.id===id);
  if(!task||!['published','in_progress','completed'].includes(status))return;
...
function renderProposalCard(proposal){ const task=state.tasks.find(t=>t.id===proposal.taskId); const statusLabel={pending:'На рассмотрении',accepted:'Принята',declined:'Отклонена',withdrawn:'Отозвана командой'}[proposal.status]||proposal.status; const statusClass={accepted:'ready',pending:'working',declined:'draft',withdrawn:'closed'}[proposal.status]; return `<article class="proposal-card ${proposal.status==='withdrawn'?'is-muted':''}"><div class="proposal-card-head"><div><p class="eyebrow muted">${escapeHtml(task?.title||'Задача')}</p><h3>${escapeHtml(proposal.team)}</h3><p class="decision-note">${escapeHtml(proposal.timeline)} ${proposal.link?`· <a href="${escapeHtml(proposal.link)}" target="_blank" rel="noreferrer">прототип</a>`:''}</p></div><div class="team-score"><span>${state.teamPoints[proposal.team]||proposal.points||0}</span> баллов</div></div><div class="proposal-grid"><div><h4>Идея</h4><p>${escapeHtml(proposal.idea)}</p></div><div><h4>План</h4><p>${escapeHtml(proposal.plan)}</p></div></div>${proposal.status==='pending'?`<div class="proposal-actions"><button class="primary-button" data-decision="accepted" data-proposal="${proposal.id}">Выбрать команду</button><button class="danger-button" data-decision="declined" data-proposal="${proposal.id}">Отклонить</button></div>`:`<div class="proposal-actions"><span class="status-pill ${statusClass}">${statusLabel}</span></div>`}</article>`; }
function updateDecision(id,decision){ const proposal=state.proposals.find(p=>p.id===id); if(!proposal||proposal.status!=='pending')return; const action=decision==='accepted'?'выбрать':'отклонить'; if(!window.confirm(`Подтвердить: ${action} отклик команды «${proposal.team}»?`))return; proposal.status=decision; const task=state.tasks.find(item=>item.id===proposal.taskId); if(decision==='accepted'){ if(!proposal.rewarded){ state.teamPoints[proposal.team]=(state.teamPoints[proposal.team]||proposal.points||0)+250; proposal.rewarded=true; } if(task){ task.status='in_progress'; task.acceptingResponses=false; } logActivity(`Команда «${proposal.team}» выбрана для задачи «${task?.title||'Задача'}»`,{taskId:proposal.taskId}); } else { logActivity(`Отклик команды «${proposal.team}» отклонён`,{taskId:proposal.taskId}); } saveState(); renderResponses(); renderNav(); showToast(decision==='accepted'?'Команда выбрана, задача переведена в работу':'Отклик отклонён'); }
...
document.querySelector('#proposal-form').addEventListener('submit',event=>{ event.preventDefault(); const fields=Object.fromEntries(new FormData(event.currentTarget).entries()); const task=state.tasks.find(item=>item.id===fields.taskId); const editing=fields.proposalId?state.proposals.find(item=>item.id===fields.proposalId):null; if(!fields.team.trim()||!fields.idea.trim()||!fields.plan.trim()||!fields.timeline.trim()){ showToast('Заполните команду, идею, план и срок'); return; } if(!task||!canAcceptResponses(task)){ showToast('Приём откликов на эту задачу закрыт'); return; } const duplicate=state.proposals.find(item=>item.taskId===fields.taskId&&item.id!==fields.proposalId&&item.status!=='withdrawn'&&item.team.trim().toLowerCase()===fields.team.trim().toLowerCase()); if(duplicate){ showToast('Эта команда уже откликнулась на задачу'); return; } if(editing){ if(editing.status!=='pending'){ showToast('Этот отклик уже нельзя изменить'); return; } Object.assign(editing,{team:fields.team.trim(),idea:fields.idea.trim(),plan:fields.plan.trim(),timeline:fields.timeline.trim(),link:fields.link.trim(),updatedAt:Date.now()}); logActivity(`Команда «${editing.team}» обновила отклик на задачу «${task.title}»`,{taskId:task.id}); } else { const proposal={taskId:fields.taskId,team:fields.team.trim(),idea:fields.idea.trim(),plan:fields.plan.trim(),timeline:fields.timeline.trim(),link:fields.link.trim(),id:`p-${Date.now()}`,status:'pending',points:state.teamPoints[fields.team.trim()]||300,createdAt:Date.now()}; state.proposals.push(proposal); if(!state.teamPoints[proposal.team])state.teamPoints[proposal.team]=300; logActivity(`Команда «${proposal.team}» отправила отклик на задачу «${task.title}»`,{taskId:task.id}); } saveState(); proposalDialog.close(); document.body.classList.remove('dialog-open'); event.currentTarget.reset(); event.currentTarget.elements.team.readOnly=false; renderNav(); if(currentView==='my-responses')renderMyResponses(); showToast(editing?'Изменения отклика сохранены':'Отклик отправлен. Решение остаётся за бизнесом'); });
